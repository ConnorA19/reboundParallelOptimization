...
struct reb_simulation* mockUnbalancedShearingSheetSimulation(int collisionDetectionType, int numParticles){
    struct reb_simulation* r = reb_simulation_create();
    // Setup constants
    r->opening_angle2    = .5;                  // This determines the precission of the tree code gravity calculation.
    r->integrator        = REB_INTEGRATOR_SEI;
...
void mockRandomShearingSheetSimulation(struct reb_simulation** r1out, struct reb_simulation** r2out, int collisionDetectionType, int numParticles, double boxsizeIn, int ghostx, int ghosty, int ghostz){
    struct reb_simulation* r1 = reb_simulation_create();
    struct reb_simulation* r2 = reb_simulation_create();
    // Setup constants
    r1->opening_angle2    = .5;                  // This determines the precission of the tree code gravity calculation.
    ...
    r2->opening_angle2    = .5;                  // This determines the precission of the tree code gravity calculation.
    ...
    // When two particles collide and the relative velocity is zero, the might sink into each other in the next time step.
    // By adding a small repulsive velocity to each collision, we prevent this from happening.
    r1->minimum_collision_velocity = particle_radius_min*OMEGA*0.001;  // small fraction of the shear accross a particle
    r2->minimum_collision_velocity = particle_radius_min*OMEGA*0.001;  // small fraction of the shear accross a particle
    ...
    for(int i =0; i < numParticles; i++){
        ...
        pt1.az         = 0;
        pt2.az = pt1.az;
        double radius     = reb_random_powerlaw(r1, particle_radius_min,particle_radius_max,particle_radius_slope);
        pt1.r         = radius;                        // m
        pt2.r = pt1.r;
        double        particle_mass = particle_density*4./3.*M_PI*radius*radius*radius;
...
void test_CollisionSearchCompare(int particleCount){
    struct reb_simulation* r1 = mockShearingSheetSimulation(REB_COLLISION_TREE, particleCount);
    struct reb_simulation* r2 = mockShearingSheetSimulation(REB_COLLISION_TREE, particleCount);
...
void test_CollisionSearchCompareRandom(int particleCount, double boxSize, int ghostx, int ghosty, int ghostz){
    struct reb_simulation* r1Ptr = NULL;
    struct reb_simulation* r2Ptr = NULL;
    mockRandomShearingSheetSimulation(&r1Ptr, &r2Ptr, REB_COLLISION_TREE, particleCount, boxSize, ghostx, ghosty, ghostz);
    ...
    for (int a = 0; a < 2; a++){
        for (int b = 0; b < 2; b++){
            for (int c = 0; c < 2; c++){
                for (int d = 0; d < 2; d++){
                    for (int e = 0; e < 2; e++){
                        time_CollisionSearchCompareRandom(numParticles2[a], boxSize2[b], ghostx2[c], ghosty2[d], ghostz2[e], iterations, timeresults);
                    }
                }
            }
        }
