...
#endif // _WIN32
const int reb_max_messages_length =
    1024; // needs to be constant expression for array size
const int reb_N_max_messages = 10;
const char *reb_build_str =
    __DATE__ " " __TIME__; // Date and time build string.
const char *reb_version_str =
    "4.4.11"; // **VERSIONLINE** This line gets updated automatically. Do not
              // edit manually.
const char *reb_githash_str = STRINGIFY(
    GITHASH); // This line gets updated automatically. Do not edit manually.
...
void reb_simulation_step(struct reb_simulation *const r) {
  ...
  if (r->tree_needs_update || r->gravity == REB_GRAVITY_TREE ||
  ...
  if (r->tree_root != NULL && r->gravity == REB_GRAVITY_TREE) {
    // Update center of mass and quadrupole moments in tree in preparation of
    // force calculation.
    reb_simulation_update_tree_gravity_data(r);
#ifdef MPI
    // Prepare essential tree (and particles close to the boundary needed for
    // collisions) for distribution to other nodes.
  ...
  r->walltime_last_step = time_end.tv_sec - time_beginning.tv_sec +
  ...
  if (r->walltime_last_steps_sum > 0.1) {
    r->walltime_last_steps =
        r->walltime_last_steps_sum / r->walltime_last_steps_N;
...
void reb_message(struct reb_simulation *const r, char type,
...
void reb_simulation_warning(struct reb_simulation *const r,
                            const char *const msg) {
  reb_message(r, 'w', msg);
}

void reb_simulation_error(struct reb_simulation *const r,
                          const char *const msg) {
  // reb_message(r, 'e', msg);
}
...
int reb_simulation_get_next_message(struct reb_simulation *const r,
                                    char *const buf) {
  if (r->messages) {
...
static int
reb_simulation_error_message_waiting(struct reb_simulation *const r) {
  if (r->messages) {
...
void reb_simulation_configure_box(struct reb_simulation *const r,
                                  const double root_size, const int N_root_x,
                                  const int N_root_y, const int N_root_z) {
  ...
  if (r->N_root_x <= 0 || r->N_root_y <= 0 || r->N_root_z <= 0) {
    reb_exit("Number of root boxes must be greater or equal to 1 in each "
             "direction.");
  }
}
#ifdef MPI
void reb_mpi_init(struct reb_simulation *const r) {
  ...
  // Make sure domain can be decomposed into equal number of root boxes per
  // node.
  if ((r->N_root / r->mpi_num) * r->mpi_num != r->N_root) {
    if (r->mpi_id == 0)
      fprintf(stderr,
              "ERROR: Number of root boxes (%d) not a multiple of mpi nodes "
              "(%d).\n",
              r->N_root, r->mpi_num);
    exit(-1);
  }
...
int reb_simulation_reset_function_pointers(struct reb_simulation *const r) {
  int wasnotnull = 0;
  if (r->coefficient_of_restitution || r->collision_resolve ||
      r->additional_forces || r->heartbeat || r->pre_timestep_modifications ||
      r->post_timestep_modifications || r->free_particle_ap ||
      r->extras_cleanup) {
    wasnotnull = 1;
  }
...
void reb_simulation_copy_with_messages(
    struct reb_simulation *r_copy, struct reb_simulation *r,
...
char *reb_simulation_diff_char(struct reb_simulation *r1,
...
int reb_simulation_diff(struct reb_simulation *r1, struct reb_simulation *r2,
                        int output_option) {
  ...
  int ret =
      reb_binary_diff(bufp1, sizep1, bufp2, sizep2, NULL, NULL, output_option);
...
struct reb_simulation *reb_simulation_copy(struct reb_simulation *r) {
  ...
  enum reb_simulation_binary_error_codes warnings =
      REB_SIMULATION_BINARY_WARNING_NONE;
...
void reb_simulation_init(struct reb_simulation *r) {
  ...
  // Integrators
  // ********** WHFAST
  // the defaults below are chosen to safeguard the user against spurious
  // results, but will be slower and less accurate
  ...
  // Tree parameters. Will not be used unless gravity or collision search makes
  // use of tree.
...
int reb_check_exit(struct reb_simulation *const r, const double tmax,
  ...
  if (r->status <= REB_STATUS_SINGLE_STEP) {
    if (r->status == REB_STATUS_SINGLE_STEP) {
      ...
      // This allows an arbitrary number of steps before the simulation is
      // paused
      r->status++;
    }
  }
  ...
  const double dtsign =
      copysign(1., r->dt); // Used to determine integration direction
  ...
  if (r->status >= 0) {
    ...
    if (r->exact_finish_time == 1) {
      if ((r->t + r->dt) * dtsign >=
          tmax * dtsign) { // Next step would overshoot
        if (r->t == tmax) {
          r->status = REB_STATUS_SUCCESS;
        } else if (r->status == REB_STATUS_LAST_STEP) {
          double tscale =
              1e-12 * fabs(tmax); // Find order of magnitude for time
          ...
          reb_simulation_synchronize(r);
          if (r->dt_last_done != 0.) { // If first timestep is also last, do not
                                       // use dt_last_done (which would be 0.)
            *last_full_dt =
                r->dt_last_done; // store last full dt before decreasing the
                                 // timestep to match finish time
          }
          r->dt = tmax - r->t;
        }
      } else {
    ...
    if (!r->N_odes) {
      ...
      if (r->integrator != REB_INTEGRATOR_BS) {
        reb_simulation_warning(
            r, "No particles found. Will exit. Use BS integrator to integrate "
               "user-defined ODEs without any particles present.");
        r->status = REB_STATUS_NO_PARTICLES; // Exit now.
      }
    }
  }
...
static void *reb_simulation_integrate_raw(void *args) {
  ...
  if (thread_info->tmax != r->t) {
    int dt_sign = (thread_info->tmax > r->t)
                      ? 1.0
                      : -1.0; // determine integration direction
    r->dt = copysign(r->dt, dt_sign);
  }

  double last_full_dt =
      r->dt; // need to store r->dt in case timestep gets artificially shrunk to
             // meet exact_finish_time=1
  r->dt_last_done = 0.; // Reset in case first timestep attempt will fail

  if (r->testparticle_hidewarnings == 0 &&
      reb_particle_check_testparticles(r)) {
    reb_simulation_warning(r,
                           "At least one test particle (type 0) has finite "
                           "mass. This might lead to unexpected behaviour. Set "
                           "testparticle_hidewarnings=1 to hide this warning.");
  }
  if (r->status != REB_STATUS_PAUSED &&
      r->status !=
          REB_STATUS_SCREENSHOT) { // Allow simulation to be paused initially
    r->status = REB_STATUS_RUNNING;
  }
  reb_run_heartbeat(r);
#ifdef __EMSCRIPTEN__
  ...
  reb_simulation_synchronize(r);
  if (r->exact_finish_time == 1) { // if finish_time = 1, r->dt could have been
                                   // shrunk, so set to the last full timestep
    r->dt = last_full_dt;
  }
  if (r->simulationarchive_filename) {
    reb_simulationarchive_heartbeat(r);
...
enum REB_STATUS reb_simulation_integrate(struct reb_simulation *const r,
                                         double tmax) {
  ...
  if (r->display_data == NULL) {
    ...
    reb_display_init(
        r); // Will return. Display routines running in animation_loop.
  }
  ...
  if (pthread_create(&r->display_data->compute_thread, NULL,
                     reb_simulation_integrate_raw, &thread_info)) {
    reb_simulation_error(r, "Error creating compute thread.");
  }

  reb_display_init(r); // Display routines need to run on main thread. Will not
                       // return until r->status<0.
