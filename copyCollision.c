...
//For parallelized function
                ...
                #pragma omp parallel
                {
                    #pragma omp single
                    {
                        ...
                        struct reb_collision** local_collisions = calloc(nthreads, sizeof(*local_collisions));
                        int* local_counts = calloc(nthreads, sizeof(*local_counts));
                        int* local_capacities = calloc(nthreads, sizeof(*local_capacities));
                    }
                }
#endif //OPENMP
                // Loop over all particles
#pragma omp parallel for schedule(guided)
                ...
                r->collisions = realloc(
                    r->collisions,
                    total * sizeof(struct reb_collision)
                );
                int offset = 0;
                for (int t=0; t<nthreads; t++){
                    memcpy(&r->collisions[offset],
                            local_collisions[t],
                            local_counts[t]*sizeof(struct reb_collision));
                        ...
                        for (int gby=-N_ghost_ycol; gby<=N_ghost_ycol; gby++){
                            for (int gbz=-N_ghost_zcol; gbz<=N_ghost_zcol; gbz++){
                                ...
                                for (int ri=0;ri<r->N_root;ri++){
                                    ...
                                    if (rootcell!=NULL){
#ifdef OPENMP
                                        reb_tree_check_for_overlapping_trajectories_in_cell_omp(r, gb, gbunmod,ri,p1_r,p1_r_plus_dtv,&collision_nearest,rootcell,maxdrift, local_collisions, local_counts, local_capacities);
#else //Not OPENMP
        ...
        if (condition){
            ...
            if (local_counts[tid] == local_capacities[tid]){
                ...
                local_collisions[tid] = realloc(
                    ...
                    local_capacities[tid] * sizeof(struct reb_collision)
                );
            }
    ...
    if (c->pt>=0){
        ...
        if (r2 < rp*rp ){
            for (int o=0;o<8;o++){
                struct reb_treecell* d = c->oct[o];
                if (d!=NULL){
                    reb_tree_check_for_overlapping_trajectories_in_cell_omp(r, gb,gbunmod,ri,p1_r,p1_r_plus_dtv,collision_nearest,d,maxdrift, local_collisions, local_counts, local_capacities);
                }
            }
        }
    }
